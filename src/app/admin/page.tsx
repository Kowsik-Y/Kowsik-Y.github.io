...
export default async function AdminDashboard() {
    ...
    const CARDS = [
        { label: "Projects", value: counts.projects, icon: FolderKanban, href: "/admin/projects", color: "text-violet-600 dark:text-violet-400 bg-violet-500/15" },
    ...
    return (
        <div>
            ...
            <p className="text-muted-foreground/80 text-sm mb-8">Overview of your portfolio content.</p>

            {/* Profile quick-edit card */}
            <div className="glass-card p-5 mb-8 flex items-center gap-5">
                <div className="relative w-16 h-16 rounded-full overflow-hidden ring-4 ring-violet-500/30 shrink-0 bg-foreground/5">
                    ...
                    {!counts.profile?.photoUrl && (
                        <div className="w-full h-full flex items-center justify-center">
                            <User size={24} className="text-muted-foreground/80" />
                        </div>
                    )}
                </div>
                <div className="flex-1 min-w-0">
                    <p className="font-semibold text-foreground truncate">{counts.profile?.name || "Kowsik Y"}</p>
                    <p className="text-sm text-violet-600 dark:text-violet-300 truncate">{counts.profile?.title || "AI & ML Engineer"}</p>
                    <p className="text-xs text-muted-foreground/80 mt-0.5 line-clamp-1">{counts.profile?.bio || "No bio set"}</p>
                </div>
                <Link
                    href="/admin/profile"
                    className="shrink-0 flex items-center gap-1.5 px-4 py-2 rounded-lg bg-violet-600 hover:bg-violet-500 text-white dark:text-white text-sm font-medium transition-colors"
                >
                    <Pencil size={14} />
                    Edit Profile
                </Link>
